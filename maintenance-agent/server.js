...
app.post("/backup/create",async(req,res)=>{let dbSnapshot="";try{
  ...
  const stamp=new Date().toISOString().replace(/[:.]/g,"-"),automaticOperational=scope==="operational"&&req.body?.automatic===true,name=scope==="operational"?`${automaticOperational?"auto":"manual"}-operational-${stamp}.zip`:`classroom-hub-${scope}-${stamp}.zip`,dest=path.join(BACKUP_DIR,name),zip=new AdmZip();
...
app.get("/backups/retention",(_req,res)=>{try{const items=fs.readdirSync(BACKUP_DIR).filter(n=>n.endsWith(".zip")).map(n=>statInfo(path.join(BACKUP_DIR,n),BACKUP_DIR)).sort((a,b)=>b.modifiedAt.localeCompare(a.modifiedAt)),automaticOperational=items.filter(x=>/^(?:auto-operational-|classroom-hub-operational-)/.test(x.name)),pre=items.filter(x=>/^pre-/.test(x.name));res.json({ok:true,total:items.length,policy:{automaticOperationalKeep:3,preKeep:1},automaticOperational:{count:automaticOperational.length,bytes:automaticOperational.reduce((a,x)=>a+Number(x.size||0),0),items:automaticOperational},pre:{count:pre.length,bytes:pre.reduce((a,x)=>a+Number(x.size||0),0),items:pre}})}catch(e){res.status(500).json({ok:false,error:e.message})}});
app.post("/backups/retention",async(req,res)=>{try{const automaticKeep=Number(req.body?.automaticKeep??3),preKeep=Number(req.body?.preKeep??1);if(!Number.isInteger(automaticKeep)||automaticKeep<1||automaticKeep>250||!Number.isInteger(preKeep)||preKeep<1||preKeep>250)return res.status(400).json({ok:false,error:"Retention values must be whole numbers from 1 to 250"});if(String(req.body?.confirm||"")!=="PRUNE_AUTOMATIC_BACKUPS")return res.status(400).json({ok:false,error:"Explicit confirmation required"});let pinned="";try{const job=await hostAgentRequest("GET","/app-updates/job");if(job.revertAvailable===true)pinned=String(job.backupName||"")}catch{}const all=fs.readdirSync(BACKUP_DIR).filter(n=>n.endsWith(".zip")).map(n=>statInfo(path.join(BACKUP_DIR,n),BACKUP_DIR)).sort((a,b)=>b.modifiedAt.localeCompare(a.modifiedAt)),automatic=all.filter(x=>/^(?:auto-operational-|classroom-hub-operational-).*\.zip$/.test(x.name)),pre=all.filter(x=>/^pre-.*\.zip$/.test(x.name));const doomed=[...automatic.slice(automaticKeep),...pre.slice(preKeep)].filter((x,i,a)=>x.name!==pinned&&a.findIndex(y=>y.name===x.name)===i);let bytes=0;for(const x of doomed){const p=path.join(BACKUP_DIR,x.name);bytes+=fs.statSync(p).size;fs.unlinkSync(p)}res.json({ok:true,automaticKeep,preKeep,pinned:pinned||null,removed:doomed.length,bytesFreed:bytes,remaining:{automatic:automatic.length-doomed.filter(x=>automatic.some(a=>a.name===x.name)).length,pre:pre.length-doomed.filter(x=>pre.some(a=>a.name===x.name)).length}})}catch(e){res.status(500).json({ok:false,error:e.message})}});
