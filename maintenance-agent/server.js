...
async function writeOperationalZipStreaming(dest,dbSnapshot,manifest){
  ...
  try{
    ...
    await run("zip",["-q","-r","-y",partial,"classroom-hub",...exclusions.flatMap(x=>["-x",x])],{timeout:900000,cwd:path.dirname(HUB_ROOT),maxBuffer:1024*1024});
    if(fs.existsSync(Classroom_ROOT)&&fs.readdirSync(Classroom_ROOT).length){
      await run("zip",["-q","-r","-y",partial,"services"],{timeout:900000,cwd:path.dirname(Classroom_ROOT),maxBuffer:1024*1024});
    }
