...
function expandTvTargets(targets,{devices={},connection="hdbt"}={}){
  const requested=Array.isArray(targets)?targets:[];
  const rows=[];
  const seen=new Set();
  const add=(id,cfg,forcedConnection=null)=>{
    const output=Number(cfg?.avOutput||String(id||"").replace(/\D/g,""));
    if(!Number.isInteger(output)||output<1||output>8)return;
    const transport=forcedConnection||String(cfg?.avConnection||cfg?.connection||connection||"hdbt").toLowerCase();
    const normalizedConnection=transport==="hdmi"?"hdmi":"hdbt";
    const key=`${normalizedConnection}:${output}`;
    if(seen.has(key))return;
    seen.add(key);
    rows.push({id:String(id),output,connection:normalizedConnection});
  };
  const all=enabledDisplayIds(devices);
  for(const raw of requested){
    const id=String(raw||"").trim().toLowerCase();
    if(!id)continue;
    if(id==="all"){for(const key of all)add(key,devices[key]);continue}
    if(id==="hdbt-all"){for(const key of all)add(key,devices[key],"hdbt");continue}
    if(id==="hdmi-all"){for(const key of all)add(key,devices[key],"hdmi");continue}
    if(devices[id])add(id,devices[id]);
  }
  return rows;
}
...
class SchedulerClock{
  ...
  setSimulation(value){
    ...
    if(Number.isNaN(date.getTime()))throw new Error("Simulated scheduler time is invalid");
    this.simulated=date;
    this.liveCommands=false;
    this.liveExpiresAt=0;
    return this.status();
  ...
  enableLiveCommands(minutes=15){
    if(!this.simulated)throw new Error("Live simulation requires an active simulated scheduler time");
    const ttl=Math.max(1,Math.min(30,Number(minutes)||15));
    this.liveCommands=true;
    this.liveExpiresAt=Date.now()+ttl*60000;
    return this.status();
  }
  commandsAllowed(){
    if(!this.simulated)return true;
    if(this.liveCommands&&Date.now()<this.liveExpiresAt)return true;
    this.liveCommands=false;this.liveExpiresAt=0;return false;
  }
  status(){
    const active=!!this.simulated;
    return {mode:active?"simulated":"real",active,timezone:this.timezone,schedulerTime:this.now().toISOString(),observedAt:new Date().toISOString(),liveCommands:active&&this.commandsAllowed(),liveExpiresAt:this.liveExpiresAt?new Date(this.liveExpiresAt).toISOString():null};
  }
}

function occurrenceId(event,dateKey,time){
  return `${String(event?.id||"unknown")}:${String(event?.classId||"manual")}:${dateKey}:${time}`;
}
...
function makeLedger(store,{key="automation.run-ledger",limit=500}={}){
  ...
  function claim(id,details={}){
    const state=read();
    const existing=[...state.runs].reverse().find(run=>run.occurrenceId===id&&["claimed","running","succeeded"].includes(run.status));
    if(existing)return {claimed:false,existing};
    const row={occurrenceId:id,status:"claimed",...details};state.runs.push({...row,observedAt:new Date().toISOString()});write(state);return {claimed:true,row};
  }
  return {read,record,claim};
}

module.exports={
  actionResourceDomain,
  isDisplayContentAction,
  normalizeIntegerMinutes,
  expandDisplayTargets,
  expandTvTargets,
