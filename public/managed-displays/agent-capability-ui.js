...
(()=>{
  ...
  async function getCaps(id){
    const old=cache.get(id);if(old&&Date.now()-old.at<8000)return old.value;
  ...
  function set(card,op,enabled,label,title){
    ...
    b.disabled=!enabled;if(label)b.textContent=label;b.title=title||"";
  }
  function badge(name,on,detail){return `<div><strong>${name}</strong>: ${on?"Available":"Unavailable"}${detail?` · ${detail}`:""}</div>`;}
  ...
  new MutationObserver(scan).observe(root,{childList:true,subtree:true});scan();setInterval(scan,10000);
})();
