...
(function installAutomationV2(){
  ...
  function render(){
    ...
    host.innerHTML=autoSteps.map((raw,i)=>{const step=normalizeAction(raw,i);autoSteps[i]=step;const label=automationActionLabel(step.action);return `<div class="card" style="margin-top:12px;border-width:2px" data-automation-v2-action="${i}">
      <div class="top"><div><b>ACTION ${i+1} — ${esc(label)}</b><div class="muted">Every action independently decides whether it runs on each pass. A lone continual action is left in place instead of being reissued.</div></div><div class="toolbar"><button type="button" onclick="RoomGoblinAutomationV2.move(${i},-1)" ${i===0?'disabled':''}>↑</button><button type="button" onclick="RoomGoblinAutomationV2.move(${i},1)" ${i===autoSteps.length-1?'disabled':''}>↓</button><button type="button" class="danger" onclick="RoomGoblinAutomationV2.remove(${i})" ${autoSteps.length===1?'disabled':''}>Remove</button></div></div>
      ...
      ${step.executionMode==='loop'?`<div class="muted">Runs whenever the sequence returns to this action. If it becomes the only continual action left, processing stops and its current state is left in place.</div>`:step.executionMode==='repeat'?`<div class="muted">Runs on the first ${step.repeatCount} pass(es), then is skipped on later passes.</div>`:`<div class="muted">Runs on the first pass only, then is skipped if other actions keep the sequence looping.</div>`}
  ...
  async function simulate(){try{const body=v2Event(),j=await api('/api/v1/automations/draft/simulate',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({...body,id:autoId.value||undefined})});autoEditorMsg.textContent=automationValidationMessage(j,`Simulation OK • ${body.actionSequence.length} action(s) • pass-based execution`);return j}catch(e){autoEditorMsg.textContent=e.message;throw e}}
  ...
  function install(){
    ...
    window.validateEnableAutomation=async()=>{autoEnabled.value="1";const result=await simulate();if(result?.ok===false)throw Error(automationValidationMessage(result,""));const saved=await save();autoEditorMsg.textContent=automationValidationMessage(result,`Saved & enabled • ${autoSteps.length} action(s)`);return saved};
