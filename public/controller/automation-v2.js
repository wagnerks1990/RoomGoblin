...
(function installAutomationV2(){
  ...
  function install(){
    ...
    window.validateEnableAutomation=async()=>{autoEnabled.value="1";const result=await simulate();if(result?.ok===false)throw Error("Resolve validation conflicts before enabling.");const saved=await save();autoEditorMsg.textContent=`Saved & enabled • ${autoSteps.length} action(s)`;return saved};
