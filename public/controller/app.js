...
function avTvDisplayId(output){const hit=displayByAvOutput(output);return hit?.[0]||null}
...
function renderDrawerTvGroups(id){if(!id){avDrawerGroups.textContent='Assign a unique receiver to edit its groups.';return}const groups=S.avConfig?.displayGroups||{};avDrawerGroups.innerHTML=Object.entries(groups).filter(([n,m])=>n!=='all'&&!(isEmptyLegacyDisplayGroup(n,m))).map(([name,members])=>`<label><input type="checkbox" data-drawer-tv-group="${esc(name)}" ${(members||[]).includes(id)?'checked':''}>${esc(name)}</label>`).join('')||'<span class="muted">No optional groups configured.</span>'}
async function saveDrawerTvGroups(){try{const id=requireAvReceiver(Number(S.avDrawerOutput)),groups={...(S.avConfig?.displayGroups||{})};document.querySelectorAll('[data-drawer-tv-group]').forEach(cb=>{const n=cb.dataset.drawerTvGroup,m=new Set(groups[n]||[]);cb.checked?m.add(id):m.delete(id);groups[n]=[...m]});const saved=await api('/api/v1/admin/displays',{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({devices:S.avConfig?.devices||{},displayGroups:groups})});S.avConfig.displayGroups=saved.displayGroups;notify('TV groups saved.','success')}catch(e){notify(e.message,'error')}}
...
async function drawerTestImage(){try{const o=Number(S.avDrawerOutput),id=requireAvReceiver(o);if(!S.avDeviceStatus?.[id]?.online)throw Error(`${avOutputLabel(o)} receiver ${id} is offline`);await sendTvTestImage(id,o);avDrawerToolStatus.textContent=`Test image sent to ${id}`;notify(`Test image sent to ${avOutputLabel(o)}.`,'success')}catch(e){avDrawerToolStatus.textContent=e.message;notify(e.message,'error')}}
...
function requireAvReceiver(output){const id=avTvDisplayId(output);if(!id)throw Error('No unique receiver assigned. Open Edit Receivers & Groups to review the output mapping.');return id}
