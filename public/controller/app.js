...
const CLASSROOM_HUB_VERSION='1.0.0-alpha.84';
    ...
    return `<div class="displayOverviewCard">
      ...
      <div class="displayOverviewBody">
        ...
        <div class="toolbar" style="margin-top:10px">
          ...
          <button onclick="window.open(${inlineJsArg(`/display/${encodeURIComponent(id)}?preview=1`)},'_blank','noopener')">Preview</button>
        </div>
      </div>
    </div>`;
...
function syncMediaSessionTargets(){
...
function ensureMediaSessionControls(){
  ...
  const panel=document.createElement('div');panel.id='mediaSessionControls';panel.className='panel';panel.innerHTML=`<h3 style="margin-top:0">Live Video Playback</h3><div class="muted">Control the video element already playing on a receiver. Volume, pause, seek, and playback-rate changes do not reload the MP4.</div><div class="grid2" style="margin-top:10px"><label>Display<select id="mediaSessionTarget"></select></label><label>Playback Rate<select id="mediaSessionRate"><option>.5</option><option>.75</option><option selected>1</option><option>1.25</option><option>1.5</option><option>2</option></select></label></div><div class="toolbar" style="margin-top:10px"><button onclick="mediaSessionCommand('play')">Play</button><button onclick="mediaSessionCommand('pause')">Pause</button><button onclick="mediaSessionCommand('restart')">Restart Clip</button><button onclick="mediaSessionCommand('stop')">Stop / Rewind</button><button onclick="mediaSessionSeekRelative(-10)">−10s</button><button onclick="mediaSessionSeekRelative(10)">+10s</button></div><label style="display:block;margin-top:10px">Position <span id="mediaSessionPositionLabel">0:00 / --:--</span><input id="mediaSessionSeek" type="range" min="0" max="1" step="0.1" value="0" style="width:100%" onchange="mediaSessionCommand('seek',{positionSeconds:Number(this.value)})"></label><label style="display:block;margin-top:10px">Volume <span id="mediaSessionVolumeLabel">100%</span><input id="mediaSessionVolume" type="range" min="0" max="100" step="1" value="100" style="width:100%" oninput="mediaSessionVolumeLabel.textContent=this.value+'%'" onchange="mediaSessionCommand('volume',{volume:Number(this.value)/100})"></label><div id="mediaSessionStatus" class="muted" style="margin-top:8px">No playback telemetry yet.</div>`;page.appendChild(panel);syncMediaSessionTargets();mediaSessionTarget.onchange=refreshMediaSessionControls;mediaSessionRate.onchange=()=>mediaSessionCommand('rate',{playbackRate:Number(mediaSessionRate.value)});clearInterval(mediaSessionRefreshTimer);mediaSessionRefreshTimer=setInterval(()=>{if(document.getElementById('media')?.classList.contains('active'))refreshMediaSessionControls()},1000)
}
...
async function refreshMediaSessionControls(){
  if(!window.mediaSessionTarget)return;syncMediaSessionTargets()
 ...
 try{
  ...
  mediaRows.innerHTML=(j.files||[]).map(f=>{
   ...
   <td><div class="toolbar">
     ${showable?`<button class="primary" onclick="showMedia(${inlineJsArg(encodeURIComponent(f.storedName))})">Show</button>`:''}
     ${(f.type==='presentation'||f.type==='document')&&f.conversionStatus!=='ready'?`<button onclick="convertMedia(${inlineJsArg(encodeURIComponent(f.storedName))})">Convert</button>`:''}
...
async function refreshPluto(){
 try{
  const [j,cfg,dev]=await Promise.all([api('/api/v1/pluto/status'),api('/api/v1/config'),api('/api/v1/devices').catch(()=>({status:{}}))]);S.pluto=j;S.avConfig=cfg||{};S.avDeviceStatus=dev.status||{};syncDisplayTargetNames(cfg.devices||{});
...
function requireAvReceiver(output){
 ...
 if(!id)throw new Error(`HDBT ${output} has no unique receiver mapping. Open Edit Receivers & Groups before using display tools or groups.`);
 if(S.avConfig?.devices?.[id]?.enabled===false)throw new Error(`Receiver ${id} is disabled. Open Edit Receivers & Groups.`);
 return id;
}
...
function renderMatrix(){
 ...
 for(let o=1;o<=8;o++){
  ...
  markup+=`<div class="mh av40TvCell"><button class="av40TvName" onclick="openAvTvDrawer(${o})"><b>${esc(L.outputs[o-1])}</b><small>HDBT ${o} • ${esc(avReceiverSummary(o,online))}</small></button><button class="av40PowerMini ${cls} ${ps.verified?'reported':''}" title="${esc(title)}" onclick="event.stopPropagation();toggleRowTvPower(${o})">⏻</button></div>`;
...
async function saveDrawerTvName(){
 ...
 try{
  ...
  if(hit){
   ...
   S.avConfig.devices=saved.devices;syncDisplayTargetNames(saved.devices);receiverSaved=true;
  }
...
function displaysByAvOutput(output){return Object.entries(S.avConfig?.devices||{}).filter(([,d])=>Number(d?.avOutput)===Number(output))}
function displayByAvOutput(output){const hits=displaysByAvOutput(output);return hits.length===1?hits[0]:null}
function avReceiverSummary(output,online){const hits=displaysByAvOutput(output);return hits.length>1?`Multiple receivers: ${hits.map(([id])=>id).join(', ')}`:hits.length?`${hits[0][0]} • Receiver ${online?'online':'offline'}`:'No receiver assigned'}
function syncDisplayTargetNames(devices){
 const changed=JSON.stringify(S.displayDevices)!==JSON.stringify(devices);S.displayDevices=devices;
 if(!changed)return;
 const selected=[...document.querySelectorAll('[data-classtarget]:checked')].map(x=>x.dataset.classtarget);
 renderClassDefaultTargets(selected.length?selected:['all']);populateDisplaySelect(window.mediaTarget);
 if(window.autoTargets&&window.autoAction&&!autoAction.value.startsWith('govee.'))renderAutoTargets([...document.querySelectorAll('[data-autotarget]:checked')].map(x=>x.dataset.autotarget));
}
  ...
  classDefaultTargets.innerHTML=classDisplayTargets().map(([id,label])=>`
    <label style="display:inline-flex;gap:7px;align-items:center;margin:4px 12px 4px 0">
      <input type="checkbox" data-classtarget="${id}" ${ids.includes(id)?'checked':''}
             onchange="classTargetChanged(${inlineJsArg(id)})">
      ${esc(label)}
    </label>
  `).join('');
...
function variableButtons(targetId){
  return `<div class="toolbar" style="margin:4px 0 8px">${automationVariables.map(v=>`<button type="button" onclick="insertAutomationVariable(${inlineJsArg(targetId)},${inlineJsArg(v)})">${v}</button>`).join('')}</div>`;
}
    ...
    <div class="toolbar" style="margin-top:6px">
      ${values.map(([id,label])=>`<label style="display:flex;gap:6px;align-items:center">
        <input type="checkbox" data-steptarget="${i}" data-target="${esc(id)}" ${selected.includes(id)?'checked':''}
          onchange="updateAutomationStepTargets(${i},${inlineJsArg(id)},this.checked)">
        ${esc(label)}
      </label>`).join('')}
    </div>
  </div>`;
...
async function loadAdminConfiguration(){
  try{
    ...
    await Promise.all([loadAdminSecrets(),loadAdminProfiles(),loadAdminUsers(),loadDisplayCredentialSecurity(),loadCloudflareSettings()]);
...
let CLOUDFLARE_STATE=null;
function renderCloudflareAuthMode(){const global=cfgCloudflareAuthMode.value==='global';document.querySelectorAll('.cfgCloudflareGlobal').forEach(x=>x.style.display=global?'':'none');cfgCloudflareTokenField.style.display=global?'none':''}
function syncCloudflareAccessDomainState(){cfgCloudflareAccessDomain.disabled=!(cfgCloudflareAccess.checked||cfgCloudflareMusicAssistant.checked)}
function cloudflareBody(){
  const body={authMode:cfgCloudflareAuthMode.value,apiEmail:cfgCloudflareApiEmail.value.trim(),zone:cfgCloudflareZone.value.trim(),hostname:cfgCloudflareHostname.value.trim(),tunnelName:cfgCloudflareTunnelName.value.trim(),alwaysUseHttps:cfgCloudflareHttps.checked,automaticHttpsRewrites:cfgCloudflareRewrites.checked,http3:cfgCloudflareHttp3.checked,brotli:cfgCloudflareBrotli.checked,accessEnabled:cfgCloudflareAccess.checked,accessEmailDomain:cfgCloudflareAccessDomain.value.trim(),musicAssistantPublicEnabled:cfgCloudflareMusicAssistant.checked,musicAssistantHostname:cfgCloudflareMusicHostname.value.trim(),replaceConflictingDns:cfgCloudflareReplaceDns.checked,adoptExistingTunnel:cfgCloudflareAdoptTunnel.checked};
  if(cfgCloudflareApiToken.value)body.apiToken=cfgCloudflareApiToken.value;
  if(cfgCloudflareGlobalKey.value)body.globalKey=cfgCloudflareGlobalKey.value;
  return body;
}
function renderCloudflareState(j){
  const s=j?.settings||j||{};CLOUDFLARE_STATE=j;
  cfgCloudflareAuthMode.value=s.authMode||'token';cfgCloudflareApiEmail.value=s.apiEmail||'';cfgCloudflareZone.value=s.zone||'';cfgCloudflareHostname.value=s.hostname||'';cfgCloudflareTunnelName.value=s.tunnelName||'';cfgCloudflareHttps.checked=s.alwaysUseHttps!==false;cfgCloudflareRewrites.checked=s.automaticHttpsRewrites!==false;cfgCloudflareHttp3.checked=s.http3!==false;cfgCloudflareBrotli.checked=s.brotli!==false;cfgCloudflareAccess.checked=s.accessEnabled===true;cfgCloudflareAccessDomain.value=s.accessEmailDomain||'';cfgCloudflareMusicAssistant.checked=s.musicAssistantPublicEnabled===true;cfgCloudflareMusicHostname.value=s.musicAssistantHostname||'';syncCloudflareAccessDomainState();cfgCloudflareReplaceDns.checked=s.replaceConflictingDns===true;cfgCloudflareAdoptTunnel.checked=s.adoptExistingTunnel===true;
  ...
  const ma=j?.resources?.musicAssistant;if(ma?.enabled){const healthy=ma.dns?.proxied===true;parts.push(`<span class="pill ${healthy?'ok':'bad'}">Music Assistant HTTPS ${healthy?'ready':'pending'}</span>`);if(ma.publicUrl)parts.push(`<a href="${esc(ma.publicUrl)}" target="_blank" rel="noopener">Open Music Assistant HTTPS</a>`)}
...
async function saveCloudflareSettings(){try{const j=await api('/api/v1/admin/cloudflare',{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(cloudflareBody())});renderCloudflareState(j.settings||{});cfgCloudflareStatus.textContent='Cloudflare settings saved; credentials remain encrypted.';notify('Cloudflare settings saved.','success')}catch(e){cfgCloudflareStatus.innerHTML=`<span class="bad">${esc(e.message)}</span>`}}
async function validateCloudflareSettings(){try{cfgCloudflareStatus.textContent='Validating Cloudflare credentials and zone…';const j=await api('/api/v1/admin/cloudflare/validate',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(cloudflareBody())});cfgCloudflareStatus.innerHTML=`<span class="pill ok">Credential valid</span> <span class="pill ok">Zone ${esc(j.zone?.name||'found')}</span> ${j.warning?`<span class="pill">${esc(j.warning)}</span>`:''}`;await loadCloudflareSettings()}catch(e){cfgCloudflareStatus.innerHTML=`<span class="bad">${esc(e.message)}</span>`}}
async function provisionCloudflare(){if(!confirm('Provision or reconcile the configured Cloudflare Tunnel, proxied DNS, HTTPS settings, protected auxiliary resources, optional RoomGoblin Access policy, and local cloudflared connector? Existing unrelated Cloudflare resources will not be deleted.'))return;try{cfgCloudflareStatus.textContent='Provisioning Cloudflare and local connector…';const j=await api('/api/v1/admin/cloudflare/provision',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(cloudflareBody())});cfgCloudflareRestart.disabled=j.connector?.restartRequired!==true;const warnings=Array.isArray(j.warnings)?j.warnings:[];cfgCloudflareStatus.innerHTML=`<span class="pill ok">Tunnel ready</span> <span class="pill ok">DNS ready</span> <span class="pill ok">Connector ${j.connector?.installed?'installed':'not installed'}</span> ${j.access?.enabled?'<span class="pill ok">Access protected</span>':''} ${warnings.length?`<span class="pill bad">${esc(warnings.length+' edge setting(s) need attention')}</span>`:''} <a href="${esc(j.publicUrl)}" target="_blank" rel="noopener">Open HTTPS URL</a>${j.resources?.musicAssistant?.publicUrl?` <a href="${esc(j.resources.musicAssistant.publicUrl)}" target="_blank" rel="noopener">Open Music Assistant HTTPS</a>`:''}${warnings.length?`<div class="bad" style="margin-top:8px">${warnings.map(esc).join('<br>')}</div>`:''}`;notify(warnings.length?'Cloudflare tunnel is ready, but some edge settings could not be applied. Review the status details.':'Cloudflare provisioning completed. Restart RoomGoblin once to activate trusted proxy handling.',warnings.length?'warning':'success')}catch(e){cfgCloudflareStatus.innerHTML=`<span class="bad">${esc(e.message)}</span>`;notify(e.message,'error')}}
async function restartRoomGoblinForCloudflare(){if(!confirm('Restart only the RoomGoblin application container now? Classroom-managed device services remain separate. The controller will briefly reconnect.'))return;try{cfgCloudflareStatus.textContent='Scheduling RoomGoblin restart…';await api('/api/v1/admin/cloudflare/restart',{method:'POST',headers:{'Content-Type':'application/json'},body:'{}'});setTimeout(()=>location.reload(),4000)}catch(e){cfgCloudflareStatus.innerHTML=`<span class="bad">${esc(e.message)}</span>`}}
...
let RECEIVERCFG=null,receiverSettingsLoading=false;
async function loadReceiverSettings(){
 if(RECEIVERCFG||receiverSettingsLoading)return;
 receiverSettingsLoading=true;cfgDisplaysMsg.textContent='Loading receivers…';
 try{RECEIVERCFG=await api('/api/v1/config');renderAdminDisplayEditor();cfgDisplaysMsg.textContent='';}
 catch(e){cfgDisplaysMsg.textContent=`Could not load receivers: ${e.message}`}
 finally{receiverSettingsLoading=false}
}
async function reloadReceiverSettings(){if(RECEIVERCFG&&!confirm('Discard unsaved receiver and group edits and reload saved settings?'))return;RECEIVERCFG=null;await loadReceiverSettings()}
function openReceiverSettings(){closeAvTvDrawer();receiverSettings.open=true;loadReceiverSettings();receiverSettings.scrollIntoView({block:'start'})}
function isEmptyLegacyDisplayGroup(name,members){return /^(?:display-){2,}/.test(name)&&Array.isArray(members)&&members.length===0}
function renderAdminDisplayEditor(){
 const ds=RECEIVERCFG?.devices||{},groups=RECEIVERCFG?.displayGroups||{};
 cfgDisplayEditor.innerHTML=Object.entries(ds).map(([id,d])=>`<div class="receiverRow" data-display-id="${esc(id)}"><div class="receiverId"><b>Receiver ID:</b> <code>${esc(id)}</code></div><label>Name<input data-f="name" value="${esc(d.name||id)}"></label><label>Enabled<select data-f="enabled"><option value="1" ${d.enabled!==false?'selected':''}>Yes</option><option value="0" ${d.enabled===false?'selected':''}>No</option></select></label><label>AV Output<input data-f="avOutput" type="number" min="1" max="64" value="${esc(d.avOutput??'')}" placeholder="Not assigned"></label><label>Lighting<input data-f="lightingAlias" value="${esc(d.lightingAlias||'')}"></label><label>Tags<input data-f="tags" value="${esc((d.tags||[]).join(', '))}"></label><button class="danger" onclick="removeAdminDisplay(${inlineJsArg(id)})">Remove Receiver</button></div>`).join('');
 ...
 const outputs=new Map();for(const [id,d] of Object.entries(ds)){if(d.avOutput){const n=Number(d.avOutput);outputs.set(n,[...(outputs.get(n)||[]),id])}}
 const shared=[...outputs].filter(([,ids])=>ids.length>1).map(([n,ids])=>`Output ${n}: ${ids.join(', ')}`);
 cfgMappingWarnings.textContent=shared.length?`Shared output assignments — review if unintended. ${shared.join('; ')}. Receiver tools need a unique assignment; routing still works.`:'';
}
function adminGroupCard(name,members,devices){return `<div class="groupCard" data-group="${esc(name)}"><div style="display:flex;justify-content:space-between;gap:8px"><b>${esc(name)}</b>${name==='all'?'<span class="pill">Core</span>':`<button class="danger" onclick="removeAdminDisplayGroup(${inlineJsArg(name)})">Remove</button>`}</div><div class="checkGrid">${Object.keys(devices).map(id=>`<label class="checkItem"><input type="checkbox" value="${esc(id)}" ${(members||[]).includes(id)?'checked':''}>${esc(devices[id]?.name||id)} (${esc(id)})</label>`).join('')}</div></div>`}
function collectAdminDisplays(){
 if(!RECEIVERCFG)throw Error('Load receiver settings first.');
 const old=RECEIVERCFG.devices||{},devices={};
 cfgDisplayEditor.querySelectorAll('[data-display-id]').forEach(row=>{const id=row.dataset.displayId,d={...(old[id]||{})};d.name=row.querySelector('[data-f="name"]').value.trim()||id;d.enabled=row.querySelector('[data-f="enabled"]').value==='1';const av=row.querySelector('[data-f="avOutput"]').value;d.avOutput=av===''?null:Number(av);if(d.avOutput!==null&&(!Number.isInteger(d.avOutput)||d.avOutput<1||d.avOutput>64))throw Error(`Invalid AV output for ${id}. Use 1–64 or leave blank.`);d.lightingAlias=row.querySelector('[data-f="lightingAlias"]').value.trim()||null;d.tags=row.querySelector('[data-f="tags"]').value.split(',').map(x=>x.trim()).filter(Boolean);devices[id]=d});
 ...
 return {...RECEIVERCFG,devices,displayGroups};
}
async function saveAdminDisplaysStructured(){try{const v=collectAdminDisplays();const j=await api('/api/v1/admin/displays',{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({devices:v.devices,displayGroups:v.displayGroups})});RECEIVERCFG={...v,devices:j.devices,displayGroups:j.displayGroups};S.avConfig=RECEIVERCFG;syncDisplayTargetNames(j.devices);syncAvOutputLabelsFromDisplays();renderMatrix();renderAdminDisplayEditor();cfgDisplaysMsg.textContent='Receivers and groups saved.';}catch(e){cfgDisplaysMsg.textContent=`Not saved: ${e.message}`}}
function removeAdminDisplay(id){if(!RECEIVERCFG||!confirm(`Remove display ${id}? Existing schedules referencing it may need review. Save to apply.`))return;RECEIVERCFG=collectAdminDisplays();delete RECEIVERCFG.devices[id];for(const g of Object.keys(RECEIVERCFG.displayGroups||{}))RECEIVERCFG.displayGroups[g]=(RECEIVERCFG.displayGroups[g]||[]).filter(x=>x!==id);renderAdminDisplayEditor()}
function addAdminDisplayGroup(){if(!RECEIVERCFG)return;const n=cfgNewGroup.value.trim().toLowerCase().replace(/[^a-z0-9_-]/g,'-');if(!n)return;RECEIVERCFG=collectAdminDisplays();if(Object.prototype.hasOwnProperty.call(RECEIVERCFG.displayGroups,n))return alert('That group already exists.');Object.defineProperty(RECEIVERCFG.displayGroups,n,{value:[],enumerable:true,writable:true,configurable:true});cfgNewGroup.value='';renderAdminDisplayEditor()}
function removeAdminDisplayGroup(name){if(!RECEIVERCFG||name==='all'||!confirm(`Remove display group ${name}? Existing schedules referencing it may need review. Save to apply.`))return;RECEIVERCFG=collectAdminDisplays();delete RECEIVERCFG.displayGroups[name];renderAdminDisplayEditor()}
function removeEmptyLegacyDisplayGroups(){if(!RECEIVERCFG)return;const v=collectAdminDisplays(),names=Object.entries(v.displayGroups).filter(([n,m])=>isEmptyLegacyDisplayGroup(n,m)).map(([n])=>n);if(!names.length)return notify('No empty legacy groups found.');if(!confirm(`Remove these empty legacy groups: ${names.join(', ')}? Existing schedule references remain unchanged. Save to apply.`))return;for(const n of names)delete v.displayGroups[n];RECEIVERCFG=v;renderAdminDisplayEditor();cfgDisplaysMsg.textContent='Empty legacy groups removed from draft. Save to apply.'}
...
const PAGE_CAPABILITIES={overview:['classroom.read'],av:['integrations.control'],lights:['integrations.control'],esphome:['classroom.read'],display:['classroom.control'],presentations:['media.manage'],media:['media.manage'],music:['classroom.control'],lab:['lab.read'],classes:['schedule.manage'],schedules:['schedule.manage','automation.manage'],diagnostics:['diagnostics.read'],settings:['admin'],system:['admin']};
const ACTION_CAPABILITIES={
  admin:['saveAdminDisplaysStructured','addAdminDisplayGroup','removeAdminDisplayGroup','removeAdminDisplay','removeEmptyLegacyDisplayGroups','saveAdmin','loadAdmin','createLabAgentEnrollment','saveLabAgentPolicy','revokeLabAgentCredential','loadSystemManagement'],
...
// -----------------------------------------------------------------------------
let fileManagerPath='.';
let AVAILABLE_APP_RELEASE=null,APP_UPDATE_POLL=null; // AVAILABLE_APP_RELEASE now represents the latest verified main commit.
...
async function loadSystemManagement(){loadHostServices();loadCleanupPlan();resumeFullRecoveryStatus();
...
async function loadHostServices(){
  ...
  try{const j=await maintApi('/host/services');const rows=j.items||[];el.innerHTML=`<div class="toolbar" style="margin-bottom:10px"><span class="pill">${j.summary?.total||0} managed/known</span><span class="pill ok">${j.summary?.running||0} running</span><span class="pill">${j.summary?.integrated||0} integrated</span><span class="pill ${j.summary?.review?'bad':''}">${j.summary?.review||0} review</span></div><table><thead><tr><th>Service</th><th>Ownership</th><th>State</th><th>Purpose</th><th>Actions</th></tr></thead><tbody>${rows.map(x=>{const arg=inlineJsArg(x.name);return `<tr><td><b>${esc(x.name)}</b><br><span class="muted">${esc(x.description||'')}</span></td><td>${esc(x.owner)}<br><span class="muted">${esc(x.recommendation||'')}</span></td><td><span class="pill ${x.active==='active'?'ok':'bad'}">${esc(x.active)}</span><br><span class="muted">${esc(x.enabled)}</span></td><td>${esc(x.purpose||'')}</td><td><div class="toolbar"><button onclick="hostServiceAction(${arg},'restart')">Restart</button><button onclick="hostServiceLogs(${arg})">Logs</button>${x.protected?'':`<button onclick="hostServiceAction(${arg},'start')">Start</button><button onclick="hostServiceAction(${arg},'stop')">Stop</button><button onclick="hostServiceAction(${arg},'enable')">Enable</button><button onclick="hostServiceAction(${arg},'disable')">Disable</button>`}</div></td></tr>`}).join('')}</tbody></table>`}catch(e){el.innerHTML=`<div class="bad">${esc(e.message)}</div>`}
}
...
async function hostServiceLogs(name){try{const j=await maintApi(`/host/service/${encodeURIComponent(name)}/logs?tail=300`);const out=document.getElementById('maintLogOutput');if(out){out.textContent=j.text||'';revealControllerOutput(out,true)}}catch(e){alert(e.message)}}
...
async function loadManagedContainers(){
  try{const j=await maintApi('/docker/containers');const rows=j.containers||[];managedContainers.innerHTML=`<table><thead><tr><th>Name</th><th>Image</th><th>Status</th><th>Network</th><th>Ports / listeners</th><th>Actions</th></tr></thead><tbody>${rows.map(c=>{const name=c.Names||c.Name||'',arg=inlineJsArg(name);return `<tr><td>${esc(name)}</td><td>${esc(c.Image||'')}</td><td>${esc(c.Status||c.State||'')}</td><td>${esc(c.Networks||'unknown')}</td><td>${esc(c.Ports||(c.Networks==='host'?'Host listeners (no port mappings)':''))}</td><td><div class="toolbar"><button onclick="managedContainerAction(${arg},'restart')">Restart</button><button onclick="managedContainerAction(${arg},'stop')">Stop</button><button onclick="managedContainerAction(${arg},'start')">Start</button></div></td></tr>`}).join('')}</tbody></table>`;maintLogContainer.innerHTML=rows.map(c=>`<option>${esc(c.Names||c.Name||'')}</option>`).join('')}catch(e){managedContainers.innerHTML=`<div class="bad">${esc(e.message)}</div>`}
}
...
async function loadRecoveryRetention(){const el=document.getElementById('recoveryRetention');if(!el)return;try{const [a,m]=await Promise.all([maintApi('/backups/retention'),maintApi('/host/migration-snapshots')]);el.innerHTML=`<b>${a.operationalAutomatic||0}</b> automatic operational backup(s) • <b>${a.pre||0}</b> pre-* safety backup(s) • <b>${m.count||0}</b> installer rollback snapshot(s) • ${fmtBytes(a.automaticBytes||0)} total automatic ZIP storage. Manual recovery backups are protected.`}catch(e){el.innerHTML=`<span class="bad">${esc(e.message)}</span>`}}
async function applyRecoveryRetention(){const keepA=Math.max(1,Math.min(50,Number(keepAutoBackups.value||3))),keepM=Math.max(1,Math.min(100,Number(keepMigrationSnapshots.value||3)));if(!confirm(`Keep the newest ${keepA} automatic operational backups, 1 pre-* safety backup, and ${keepM} installer rollback snapshots? Older automatic items will be permanently removed. Manual/full backups are not affected.`))return;try{const [a,m]=await Promise.all([maintApi('/backups/retention',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({automaticKeep:keepA,preKeep:1,confirm:'PRUNE_AUTOMATIC_BACKUPS'})}),maintApi('/host/migration-retention',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({keep:keepM,confirm:'PRUNE_MIGRATIONS'})})]);alert(`Retention applied. Removed ${a.removed||0} automatic backup(s) and ${m.removedCount||0} migration snapshot(s).`);await Promise.all([loadRecoveryRetention(),loadManagedBackups()])}catch(e){alert(e.message)}}
async function createManagedBackup(scope=backupScope.value,{download=false}={}){const sensitive=scope!=='diagnostic';if(scope==='full')return createFullRecoveryExport();if(sensitive&&!confirm('This recovery export can contain private appliance data, device identifiers, user or student records, or endpoint credentials. Store it as a secret. Continue?'))return;try{const j=await maintApi('/backup/create',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({scope,confirmSensitiveData:sensitive,confirmSecrets:false})});await loadManagedBackups();if(download)downloadManagedBackup(j.name,true,true);alert(`Export created: ${j.name}${download?'\nDownload started.':''}${j.containsSensitiveData?'\nProtect this sensitive archive.':''}`)}catch(e){alert(e.message)}}
function recoveryClientTransportAllowed(){const host=String(location.hostname||'').toLowerCase().replace(/^\[|\]$/g,'');return location.protocol==='https:'||host==='localhost'||host==='::1'||/^127(?:\.\d{1,3}){3}$/.test(host)}
async function ensureFullRecoveryTransport(){if(!recoveryClientTransportAllowed())throw Error('Encrypted full recovery requires HTTPS or a browser running directly on the RoomGoblin host.');const policy=await api('/api/v1/admin/recovery-transport');if(!policy.allowed)throw Error('The server rejected this recovery connection. Use HTTPS or open RoomGoblin locally on the host.');return policy}
function validRecoveryPassphrase(value){return typeof value==='string'&&value.length>=16&&value.length<=1024}
async function createFullRecoveryExport(event){event?.preventDefault?.();const passField=document.getElementById('fullRecoveryExportPassphrase'),confirmField=document.getElementById('fullRecoveryExportConfirm'),button=document.getElementById('fullRecoveryExportButton');let passphrase=passField?.value||'';try{if(!validRecoveryPassphrase(passphrase))throw Error('Use an encryption passphrase of at least 16 characters.');if(passphrase!==(confirmField?.value||''))throw Error('The recovery passphrases do not match.');await ensureFullRecoveryTransport();if(!confirm('A Full Recovery Export contains private appliance data, device identity, user or student records, endpoint credentials, and recovery keys. Continue?'))return;if(!confirm('RoomGoblin cannot recover this passphrase. Create the encrypted full recovery bundle now?'))return;button.disabled=true;const j=await maintApi('/backup/create',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({scope:'full',confirmSensitiveData:true,confirmSecrets:true,passphrase})});if(j.encrypted!==true||!String(j.name||'').endsWith('.rgbak'))throw Error('The maintenance service did not return an encrypted recovery bundle.');await loadManagedBackups();downloadManagedBackup(j.name,true,true);notify('Encrypted full recovery bundle created. Keep it with its passphrase in a protected location.','success')}catch(e){notify(e.message,'error')}finally{passphrase='';if(passField)passField.value='';if(confirmField)confirmField.value='';if(button)button.disabled=false}}
function managedBackupCapabilities(item){const c=item.capabilities||{},parts=[[c.database,'SQLite database'],[c.data,'RoomGoblin data'],[c.services,'Managed services'],[c.secrets,'Recovery secrets']].filter(([available])=>available).map(([,label])=>`<span class="pill">${label}</span>`),complete=c.database&&c.data&&c.services&&c.secrets;return `<div class="toolbar" style="margin-top:0">${complete?'<span class="pill bad">Full Recovery bundle</span>':''}${parts.join('')||'<span class="pill">No restorable application state</span>'}</div>`}
function managedBackupRestoreButtons(item){const arg=inlineJsArg(item.name),modes=new Set(item.restoreModes||[]),full=modes.has('full-recovery')||String(item.name||'').endsWith('.rgbak'),exportOnly=!full&&(item.capabilities?.services||item.capabilities?.secrets);return `${full?`<button class="danger" onclick="unlockFullRecovery(${arg})">Unlock Full Recovery</button>`:''}${modes.has('configuration')?`<button onclick="restoreManagedBackup(${arg},'configuration')">Restore Database</button>`:''}${modes.has('configuration-data')?`<button class="danger" onclick="restoreManagedBackup(${arg},'configuration-data')">Restore Database + RoomGoblin Data</button>`:''}${modes.size||full?'':'<span class="muted">No application restore available</span>'}${exportOnly?'<span class="muted">Secrets/services are export-only here</span>':''}`}
async function loadManagedBackups(){try{
  ...
  html+=`<table><thead><tr><th>Backup</th><th>Bundle Contents</th><th>Size</th><th>Created</th><th>Available Actions</th></tr></thead><tbody>${items.map(x=>{const arg=inlineJsArg(x.name),encrypted=x.encrypted===true||String(x.name||'').endsWith('.rgbak');return `<tr><td style="text-align:left">${esc(x.name)}<br><span class="pill ${x.containsSensitiveData?'bad':'ok'}">${encrypted?'Encrypted full recovery':x.containsSensitiveData?'Sensitive recovery data':'Support-safe diagnostic'}</span></td><td>${managedBackupCapabilities(x)}</td><td>${fmtBytes(x.size)}</td><td>${esc(new Date(x.modifiedAt).toLocaleString())}</td><td><div class="toolbar">${encrypted?'':`<button onclick="inspectManagedBackup(${arg})">Inspect</button><button onclick="restorePlanManagedBackup(${arg})">View Restore Capabilities</button>`}${managedBackupRestoreButtons(x)}<button onclick="downloadManagedBackup(${arg},${x.containsSensitiveData===true})">Download</button><button class="danger" onclick="deleteManagedBackup(${arg})">Delete</button></div></td></tr>`}).join('')}</tbody></table>`;
...
let FULL_RECOVERY_SELECTED='',FULL_RECOVERY_VERIFIED='',FULL_RECOVERY_POLL=null;
const FULL_RECOVERY_STATUS_KEY='roomgoblin.recoveryId';
function fullRecoveryBundleName(value){const cleaned=String(value||'').replace(/[^A-Za-z0-9._-]/g,'_').slice(-120);return cleaned.toLowerCase().endsWith('.rgbak')?cleaned:`roomgoblin-import-${Date.now()}.rgbak`}
function renderFullRecoveryPlan(name,plan){FULL_RECOVERY_SELECTED=String(name||'');FULL_RECOVERY_VERIFIED=plan?.fullRecoveryRestorable===true&&plan?.encrypted===true?FULL_RECOVERY_SELECTED:'';const panel=document.getElementById('fullRecoveryRestorePanel'),title=document.getElementById('fullRecoveryRestoreName'),output=document.getElementById('fullRecoveryPlan');if(panel)panel.hidden=false;if(title)title.textContent=FULL_RECOVERY_SELECTED;const capabilities=Object.entries(plan?.capabilities||{}).filter(([,available])=>available===true).map(([key])=>key).join(', ');if(output)output.textContent=FULL_RECOVERY_VERIFIED?`Authenticated recovery plan verified. Included state: ${capabilities||'full appliance state'}. Re-enter the passphrase and type the confirmation phrase to start.`:'This bundle cannot perform authenticated full recovery on this appliance.';document.getElementById('fullRecoveryRestorePassphrase')?.focus()}
function unlockFullRecovery(name){FULL_RECOVERY_SELECTED=String(name||'');FULL_RECOVERY_VERIFIED='';const panel=document.getElementById('fullRecoveryRestorePanel'),title=document.getElementById('fullRecoveryRestoreName'),output=document.getElementById('fullRecoveryPlan');if(panel)panel.hidden=false;if(title)title.textContent=FULL_RECOVERY_SELECTED;if(output)output.textContent='Enter the bundle passphrase, then choose Unlock & Verify Recovery Plan.';document.getElementById('fullRecoveryRestorePassphrase')?.focus()}
async function verifySelectedFullRecovery(){const field=document.getElementById('fullRecoveryRestorePassphrase');let passphrase=field?.value||'';try{if(!FULL_RECOVERY_SELECTED)throw Error('Select an encrypted recovery bundle first.');if(!validRecoveryPassphrase(passphrase))throw Error('Enter the bundle passphrase (at least 16 characters).');await ensureFullRecoveryTransport();const j=await maintApi(`/backup/${encodeURIComponent(FULL_RECOVERY_SELECTED)}/restore-plan`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({passphrase})});renderFullRecoveryPlan(FULL_RECOVERY_SELECTED,j.plan||{});if(!FULL_RECOVERY_VERIFIED)throw Error('The bundle is valid but is not eligible for full recovery.')}catch(e){FULL_RECOVERY_VERIFIED='';const output=document.getElementById('fullRecoveryPlan');if(output)output.textContent=e.message;notify(e.message,'error')}finally{passphrase='';if(field)field.value=''}}
async function importFullRecoveryBundle(event){event?.preventDefault?.();const fileField=document.getElementById('fullRecoveryImportFile'),passField=document.getElementById('fullRecoveryImportPassphrase'),button=document.getElementById('fullRecoveryImportButton'),file=fileField?.files?.[0];let passphrase=passField?.value||'';try{if(!file)throw Error('Choose an encrypted .rgbak recovery bundle.');if(!String(file.name||'').toLowerCase().endsWith('.rgbak'))throw Error('Full recovery imports must use the .rgbak encrypted bundle format.');if(!validRecoveryPassphrase(passphrase))throw Error('Enter the bundle passphrase (at least 16 characters).');await ensureFullRecoveryTransport();button.disabled=true;const j=await maintApi('/backup/import',{method:'POST',headers:{'Content-Type':'application/octet-stream','X-Backup-Name':fullRecoveryBundleName(file.name),'X-Recovery-Passphrase':passphrase},body:file});renderFullRecoveryPlan(j.name,j.plan||{});await loadManagedBackups();notify('Encrypted bundle authenticated and imported. Review the plan before restoring.','success')}catch(e){notify(e.message,'error')}finally{passphrase='';if(passField)passField.value='';if(fileField)fileField.value='';if(button)button.disabled=false}}
function rememberRecoveryId(value){if(!/^[A-Za-z0-9_-]{32,128}$/.test(String(value||'')))throw Error('The recovery service returned an invalid operation identifier.');sessionStorage.setItem(FULL_RECOVERY_STATUS_KEY,String(value));return String(value)}
async function startFullRecovery(){const passField=document.getElementById('fullRecoveryRestorePassphrase'),confirmField=document.getElementById('fullRecoveryConfirmation'),button=document.getElementById('fullRecoveryRestoreButton');let passphrase=passField?.value||'';try{if(!FULL_RECOVERY_SELECTED||FULL_RECOVERY_VERIFIED!==FULL_RECOVERY_SELECTED)throw Error('Unlock and verify this recovery bundle first.');if(!validRecoveryPassphrase(passphrase))throw Error('Re-enter the bundle passphrase (at least 16 characters).');if((confirmField?.value||'')!=='RESTORE_FULL_RECOVERY')throw Error('Type RESTORE_FULL_RECOVERY exactly to confirm.');await ensureFullRecoveryTransport();if(!confirm('Full recovery will stop RoomGoblin, replace appliance state, verify services and secrets, and automatically roll back if validation fails. Continue?'))return;button.disabled=true;const j=await maintApi(`/backup/${encodeURIComponent(FULL_RECOVERY_SELECTED)}/restore`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({mode:'full-recovery',confirm:'RESTORE_FULL_RECOVERY',passphrase})});const recoveryId=rememberRecoveryId(j.recoveryId);FULL_RECOVERY_SELECTED='';FULL_RECOVERY_VERIFIED='';document.getElementById('fullRecoveryRestorePanel').hidden=true;setRecoveryStatus('Full recovery accepted. RoomGoblin may disconnect temporarily while appliance state is replaced.');pollFullRecoveryStatus(recoveryId)}catch(e){notify(e.message,'error')}finally{passphrase='';if(passField)passField.value='';if(confirmField)confirmField.value='';if(button)button.disabled=false}}
// Reveal action results without changing pages or bypassing page authorization.
function revealControllerOutput(target,scroll=false){
  const output=typeof target==='string'?document.getElementById(target):target;
  if(!output)return null;
  for(let parent=output.parentElement;parent;parent=parent.parentElement){if(parent.tagName==='DETAILS')parent.open=true}
  if(scroll)output.scrollIntoView({behavior:matchMedia('(prefers-reduced-motion: reduce)').matches?'auto':'smooth',block:'center'});
  return output;
}
function setRecoveryStatus(message,kind=''){const output=document.getElementById('fullRecoveryStatus');if(!output)return;revealControllerOutput(output);output.textContent=String(message||'');output.classList.toggle('bad',kind==='error');output.classList.toggle('ok',kind==='success')}
function recoveryStatusId(){try{return sessionStorage.getItem(FULL_RECOVERY_STATUS_KEY)||''}catch{return ''}}
function forgetRecoveryId(){try{sessionStorage.removeItem(FULL_RECOVERY_STATUS_KEY)}catch{}}
function resumeFullRecoveryStatus(){const recoveryId=recoveryStatusId();if(recoveryId&&/^[A-Za-z0-9_-]{32,128}$/.test(recoveryId))pollFullRecoveryStatus(recoveryId)}
async function pollFullRecoveryStatus(recoveryId){clearTimeout(FULL_RECOVERY_POLL);try{const response=await fetch(`/api/v1/recovery-status/${encodeURIComponent(recoveryId)}`,{cache:'no-store',credentials:'omit'}),status=await response.json().catch(()=>({}));if(!response.ok)throw Error(status.error||`Recovery status HTTP ${response.status}`);const rollback=status.rollback?.attempted?`\nRollback: ${status.rollback.ok?'completed':'failed'}`:'';setRecoveryStatus(`Recovery phase: ${status.phase||'unknown'}\n${status.message||''}${rollback}`,status.running?'':status.ok?'success':'error');if(status.running){FULL_RECOVERY_POLL=setTimeout(()=>pollFullRecoveryStatus(recoveryId),3000);return}forgetRecoveryId();if(status.ok){notify('Full recovery completed. Sign in again to verify the restored appliance.','success');if(status.reloginRequired){window.AUTH_STATUS={...(window.AUTH_STATUS||{}),user:null};applyAuthUi(window.AUTH_STATUS);showLogin()}}else notify(status.rollback?.ok?'Recovery failed; the previous appliance state was restored.':'Recovery failed. Review the host recovery journal before retrying.','error')}catch(e){setRecoveryStatus(`RoomGoblin is temporarily unavailable while recovery runs. Retrying…\n${e.message}`);FULL_RECOVERY_POLL=setTimeout(()=>pollFullRecoveryStatus(recoveryId),5000)}}
async function restorePlanManagedBackup(name){try{const j=await maintApi(`/backup/${encodeURIComponent(name)}/restore-plan`);maintenanceOutput.textContent=JSON.stringify(j,null,2);revealControllerOutput(maintenanceOutput,true)}catch(e){alert(e.message)}}
function downloadManagedBackup(name,sensitive,confirmed=false){if(String(name||'').endsWith('.rgbak')&&!recoveryClientTransportAllowed())return notify('Encrypted recovery downloads require HTTPS or a browser running directly on the RoomGoblin host.','error');if(sensitive&&!confirmed&&!confirm('This backup contains sensitive recovery data. Keep it encrypted at rest and do not attach it to support tickets. Download anyway?'))return;const link=document.createElement('a');link.href=`/api/v1/maintenance/backup/${encodeURIComponent(name)}`;link.download=name;document.body.appendChild(link);link.click();link.remove()}
async function restoreManagedBackup(name,mode){const label=mode==='configuration'?'the SQLite database':'the SQLite database and RoomGoblin application data';if(!confirm(`Restore ${label} from ${name}? This controller action does not restore recovery secrets or managed-service state. A new safety backup will be created first.`))return;if(!confirm('This overwrites current application state and restarts RoomGoblin. Continue with the selected restore?'))return;try{revealControllerOutput('maintenanceOutput',true);maintenanceOutput.textContent='Restoring backup…';const j=await maintApi(`/backup/${encodeURIComponent(name)}/restore`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({mode,confirm:'RESTORE'})});maintenanceOutput.textContent=JSON.stringify(j,null,2);alert(`Restore completed and database health was verified. Safety backup: ${j.safetyBackup||'created'}`);setTimeout(()=>location.reload(),2500)}catch(e){maintenanceOutput.textContent=e.message;alert(e.message)}}
async function inspectManagedBackup(name){try{const j=await maintApi(`/backup/${encodeURIComponent(name)}/inspect`);maintenanceOutput.textContent=JSON.stringify(j,null,2);revealControllerOutput('maintenanceOutput',true)}catch(e){alert(e.message)}}
...
async function runMaintenancePreset(preset){revealControllerOutput('maintenanceOutput',true);try{const j=await maintApi('/command',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({preset})});maintenanceOutput.textContent=j.output||JSON.stringify(j,null,2)}catch(e){maintenanceOutput.textContent=e.message}}
function renderAppUpdateHistory(history=[]){appUpdateHistory.innerHTML=history.length?`<table><thead><tr><th>Time</th><th>Action</th><th>Result</th><th>Deployment</th><th>Recovery</th></tr></thead><tbody>${history.slice(0,20).map(x=>{const commit=String(x.activeCommit||x.targetCommit||'');const deployment=[x.activeVersion||x.targetRef||'',commit?commit.slice(0,12):''].filter(Boolean).join(' @ ');return `<tr><td>${esc(x.at?new Date(x.at).toLocaleString():'')}</td><td>${esc(x.action==='published'?'main':x.action||'update')}</td><td><span class="pill ${x.ok===true?'ok':x.ok===false?'bad':''}">${esc(x.phase||'')}</span><br><span class="muted">${esc(x.message||'')}</span></td><td>${esc(deployment)}</td><td>${esc(x.rollback===true?'Rolled back':x.backupName||'')}</td></tr>`}).join('')}</tbody></table>`:'<div class="muted">No application update history yet.</div>'}
async function loadAppUpdates(){try{const [settings,job]=await Promise.all([api('/api/v1/admin/app-updates/settings'),api('/api/v1/admin/app-updates/job').catch(e=>({ok:false,error:e.message,history:[]}))]);const p=settings.settings||{};appUpdateRepository.value=p.repository||'';appUpdateChannel.value='main';appUpdateInterval.value=p.checkIntervalHours||24;appUpdateStart.value=p.maintenanceStart||'02:00';appUpdateEnd.value=p.maintenanceEnd||'04:00';appUpdateAutomatic.checked=!!p.automatic;appUpdateToken.placeholder=settings.tokenConfigured?'Token stored — leave blank to keep it':'Optional for public repositories';renderAppUpdateHistory(job.history||settings.history||[]);revertAppUpdateBtn.disabled=job.revertAvailable!==true||job.running;const currentCommit=String(settings.currentCommit||job.activeCommit||'');updateOutput.textContent=job.error?job.error:`Current version: ${settings.currentVersion}\nCurrent commit: ${currentCommit?currentCommit.slice(0,12):'unknown'}\nSource: main\nPhase: ${job.phase||'idle'}\n${job.message||''}${job.targetCommit?`\nTarget commit: ${String(job.targetCommit).slice(0,12)}`:job.targetRef?`\nTarget: ${job.targetRef}`:''}${job.backupName?`\nRecovery backup: ${job.backupName}`:''}`;if(job.running){revealControllerOutput('updateOutput');pollAppUpdateJob()}else await checkAppUpdate(true)}catch(e){updateOutput.textContent=e.message}}
...
async function saveAppUpdateSettings(){clearSelectedAppRelease();try{const body={repository:appUpdateRepository.value.trim(),source:'main',automatic:appUpdateAutomatic.checked,checkIntervalHours:Number(appUpdateInterval.value||24),maintenanceStart:appUpdateStart.value,maintenanceEnd:appUpdateEnd.value};if(appUpdateToken.value)body.token=appUpdateToken.value;const j=await api('/api/v1/admin/app-updates/settings',{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});appUpdateToken.value='';appUpdateToken.placeholder=j.tokenConfigured?'Token stored — leave blank to keep it':'Optional for public repositories';notify('Main-branch update settings saved.','success');await checkAppUpdate(true)}catch(e){notify(e.message,'error')}}
async function clearAppUpdateToken(){if(!confirm('Remove the stored GitHub read token? Public repository updates will continue to work.'))return;try{const j=await api('/api/v1/admin/app-updates/settings',{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({repository:appUpdateRepository.value.trim(),source:'main',automatic:appUpdateAutomatic.checked,checkIntervalHours:Number(appUpdateInterval.value||24),maintenanceStart:appUpdateStart.value,maintenanceEnd:appUpdateEnd.value,clearToken:true})});appUpdateToken.value='';appUpdateToken.placeholder=j.tokenConfigured?'Token stored — leave blank to keep it':'Optional for public repositories';notify('Stored GitHub token removed.','success')}catch(e){notify(e.message,'error')}}
async function checkAppUpdate(silent=false){if(!silent)updateOutput.textContent='Checking trusted GitHub main for merged commits…';installAppUpdateBtn.disabled=true;try{const j=await api('/api/v1/admin/app-updates/check',{method:'POST'});AVAILABLE_APP_RELEASE=j.available||null;installAppUpdateBtn.disabled=!AVAILABLE_APP_RELEASE;const latest=String(j.latest?.shortCommit||j.latest?.commit||'unknown').slice(0,12);updateOutput.textContent=AVAILABLE_APP_RELEASE?`Current version: ${j.currentVersion}\nCurrent commit: ${String(j.shortCurrentCommit||j.currentCommit).slice(0,12)}\nSource: ${j.repository} / main\nAvailable: ${AVAILABLE_APP_RELEASE.shortCommit}\nCommits ahead: ${AVAILABLE_APP_RELEASE.commitsAhead||1}\nMerged: ${AVAILABLE_APP_RELEASE.mergedAt?new Date(AVAILABLE_APP_RELEASE.mergedAt).toLocaleString():'unknown'}\nSummary: ${AVAILABLE_APP_RELEASE.name||'Merged main update'}\nStatus: Ready to install after exact CI images are verified by the updater.`:`Current version: ${j.currentVersion}\nCurrent commit: ${String(j.shortCurrentCommit||j.currentCommit).slice(0,12)}\nSource: ${j.repository} / main\nMain commit: ${latest}\nStatus: Up to date`;return j}catch(e){AVAILABLE_APP_RELEASE=null;installAppUpdateBtn.disabled=true;updateOutput.textContent=`Update check failed: ${e.message}`;if(!silent){notify(e.message,'error');throw e}return null}}
async function installAppUpdate(){if(!AVAILABLE_APP_RELEASE)return;const commit=AVAILABLE_APP_RELEASE.commit,short=AVAILABLE_APP_RELEASE.shortCommit||String(commit).slice(0,12);if(!confirm(`Install merged main commit ${short}? A database-safe recovery backup will be created before source or services change.`))return;if(!confirm('RoomGoblin may briefly disconnect while the exact CI-built images are verified and changed components are reconciled. Failed health verification will automatically restore the previous deployment. Continue?'))return;try{const j=await api('/api/v1/admin/app-updates/install',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({commit})});updateOutput.textContent=`Main update job started.\nTarget commit: ${short}\nRecovery backup: ${j.safetyBackup||'creating'}\nThe page will keep checking progress.`;installAppUpdateBtn.disabled=true;pollAppUpdateJob()}catch(e){updateOutput.textContent=e.message}}
...
async function saveMusicAssistantConfig(){try{await api('/api/v1/music-assistant/config',{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({url:maUrl.value.trim(),browserUrl:maBrowserUrl.value.trim(),token:maToken.value||undefined,tvBridgeEnabled:true})});maToken.value='';await loadMusicAssistant()}catch(e){alert(e.message)}}
function openMusicAssistantUi(){try{const localHosts=new Set(['127.0.0.1','localhost','host.docker.internal','0.0.0.0','[::1]']);const explicit=maBrowserUrl.value.trim();if(explicit){const u=new URL(explicit);if(!['http:','https:'].includes(u.protocol)||u.username||u.password)throw Error('Music Assistant browser URL must use HTTP(S) without embedded credentials.');return window.open(u.href,'_blank','noopener,noreferrer')}const u=new URL((maUrl.value||'http://127.0.0.1:8095').trim());if(!['http:','https:'].includes(u.protocol))throw Error('HTTP(S) URL required');if(!localHosts.has(u.hostname))return window.open(u.href,'_blank','noopener,noreferrer');if(location.protocol==='http:'){u.hostname=location.hostname;return window.open(u.href,'_blank','noopener,noreferrer')}throw Error('Music Assistant has no protected browser HTTPS URL. Enable it under Cloudflare Remote HTTPS or use the LAN HTTP fallback.')}catch(e){notify(e.message,'error')}}
...
async function deployManagedModule(id,recreate=true){if(recreate&&!confirm('Recreate this integration using the reviewed RoomGoblin network topology? This interrupts the service. Verify its persistent mounts, saved configuration, and required host listeners first.'))return;try{const settings=collectManagedModuleSettings(id);const j=await maintApi(`/modules/${encodeURIComponent(id)}/deploy`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({settings,recreate})});alert(j.message||`${id} deployment completed.`);await Promise.all([loadManagedModules(),loadManagedContainers()])}catch(e){alert(e.message)}}
