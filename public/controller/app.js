...
function adminGroupCard(name,members,devices){return `<div class="groupCard" data-group="${esc(name)}"><div style="display:flex;justify-content:space-between;gap:8px"><b>${esc(name)}</b>${name==='all'?'<span class="pill">Core</span>':`<button class="danger" onclick="removeAdminDisplayGroup(${inlineJsArg(name)})">Remove</button>`}</div><div class="checkGrid">${Object.keys(devices).map(id=>`<label class="checkItem"><input type="checkbox" value="${esc(id)}" ${(members||[]).includes(id)?'checked':''}>${esc(id)}</label>`).join('')}</div></div>`}
