...
function setRecoveryStatus(message,kind=''){const output=document.getElementById('fullRecoveryStatus');if(!output)return;output.textContent=String(message||'');output.classList.toggle('bad',kind==='error');output.classList.toggle('ok',kind==='success')}
...
async function restoreManagedBackup(name,mode){const label=mode==='configuration'?'the SQLite database':'the SQLite database and RoomGoblin application data';if(!confirm(`Restore ${label} from ${name}? This controller action does not restore recovery secrets or managed-service state. A new safety backup will be created first.`))return;if(!confirm('This overwrites current application state and restarts RoomGoblin. Continue with the selected restore?'))return;try{maintenanceOutput.textContent='Restoring backup…';const j=await maintApi(`/backup/${encodeURIComponent(name)}/restore`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({mode,confirm:'RESTORE'})});maintenanceOutput.textContent=JSON.stringify(j,null,2);alert(`Restore completed and database health was verified. Safety backup: ${j.safetyBackup||'created'}`);setTimeout(()=>location.reload(),2500)}catch(e){maintenanceOutput.textContent=e.message;alert(e.message)}}
