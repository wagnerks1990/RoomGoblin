...
In **Settings → Integrations & Hardware → Veyon Classroom Computers**, use the **Additional Veyon authentication keys** panel to import or replace a named PEM, choose a preferred key, review configured key names, or remove non-default keys. Private PEM contents are encrypted in SQLite and are never returned to the browser after import.
