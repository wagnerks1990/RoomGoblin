...
Renderer `dynamic-fit-20260921-4` uses dynamic layout objects for title, subtitle, lesson body, and timer.
...
After updating the Hub, reload receiver pages and verify `window.ClassroomDisplayDiagnostics()` reports `revision: "dynamic-fit-20260921-4"`, `dynamic: true`, the expected object order, no overlap, and no fit warning for normal classroom content.
