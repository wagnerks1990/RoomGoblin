...
Operational backups intentionally exclude `data/media`; uploaded media is replaceable and must not block the mandatory pre-update backup. Normal configuration-data restore preserves the live `data/media` directory because current operational archives do not contain it. Interrupted legacy restore recovery is the compatibility exception: older safety archives may contain media, so startup rollback must restore those archived media entries. Preserve the streaming Info-ZIP/ZIP64 path and do not reintroduce whole-archive Node buffering.
