...
Cloudflare is an optional remote-access layer, never part of RoomGoblin's local classroom availability chain. Read `docs/CLOUDFLARE-TUNNEL.md` and `docs/ai/CLOUDFLARE-TUNNEL.md` before changing Cloudflare provisioning, public exposure, proxy trust, Full Recovery transport, or the connector installer.

Preserve these boundaries: API credentials stay in encrypted SQLite secret storage; tunnel connector tokens stay server-side and ultimately in `/etc/cloudflared/roomgoblin.token` mode 0600; tunnel ingress exposes only the configured RoomGoblin hostname to loopback; `TRUST_PROXY_HOPS=1` is the reviewed topology; same-name tunnel adoption and conflicting DNS takeover require explicit administrator consent; Access requires an explicit Allow selector and never replaces RoomGoblin authentication. Never publish maintenance, Host Agent, Docker, SSH, Veyon, MQTT, Music Assistant, or lab subnets through this feature. Do not make Cloudflare health gate Morning Announcements, scheduler recovery, Background Music reconciliation, display/device stability, updates, recovery, or local administration.
...
Music Assistant host-network compatibility is part of the managed deployment contract: recreated containers must keep `/data/.roomgoblin-compat/sitecustomize.py` on `PYTHONPATH` so discovery/publish selection is pinned to the default-route LAN instead of Docker/veth/VPN interfaces. Managed recreations must also launch through `/bin/sh /data/.roomgoblin-compat/wait-for-lan.sh`; that gate waits at most 60 seconds for a usable LAN IPv4 and then execs the upstream image contract `/usr/local/bin/entrypoint.sh --data-dir /data --cache-dir /data/.cache`. Do not replace the gate with an unbounded wait, a site-specific IP, or a guessed entrypoint. Adopt-without-recreate remains non-destructive.
...
Operational backups intentionally exclude `data/media`; uploaded media is replaceable and must not block the mandatory pre-update backup. Top-level `data/*.db`, `*.db-wal`, and `*.db-shm` identities are also excluded from the filesystem pass; the operational archive must contain only the separately created canonical SQLite snapshot, never stale legacy database files. Normal configuration-data restore preserves the live `data/media` directory because current operational archives do not contain it. Interrupted legacy restore recovery is the compatibility exception: older safety archives may contain media, so startup rollback must restore those archived media entries. Preserve the streaming Info-ZIP/ZIP64 path and do not reintroduce whole-archive Node buffering.
