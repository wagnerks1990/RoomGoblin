...
if [[ "$ACTION" == published ]]; then
  ...
  BACKUP_JSON="$(docker compose exec -T maintenance-agent node -e '
    fetch("http://127.0.0.1:"+(process.env.PORT||3010)+"/backup/create",{method:"POST",headers:{"content-type":"application/json","x-maintenance-token":process.env.MAINTENANCE_TOKEN},body:JSON.stringify({scope:"operational",automatic:true,confirmSensitiveData:true}),signal:AbortSignal.timeout(900000)}).then(async r=>{const j=await r.json();if(!r.ok||!j.ok)throw Error(j.error||"Operational backup failed"); const r2=await fetch("http://127.0.0.1:"+(process.env.PORT||3010)+"/backup/"+encodeURIComponent(j.name)+"/inspect",{headers:{"x-maintenance-token":process.env.MAINTENANCE_TOKEN},signal:AbortSignal.timeout(120000)});const checked=await r2.json();if(!r2.ok||checked.manifest?.databaseSnapshot!==true)throw Error(checked.error||"Missing database snapshot");console.log(JSON.stringify({name:j.name,sha256:j.sha256}))}).catch(e=>{console.error("Operational backup preflight failed:",e.message);process.exit(1)})'
  )"
