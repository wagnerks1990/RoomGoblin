...
- A loopback proxy socket is not the direct-localhost recovery exception. Any known forwarding header (including an empty value) disables that exception. Do not substitute Host/Origin as transport authentication or newly reject existing unforwarded loopback API clients using aliases. Preserve direct localhost and SSH-forwarded access.
...
- The trusted proxy must overwrite `X-Forwarded-Proto` from the original client connection. A proxy that strips all forwarding metadata cannot be distinguished from a direct client; that configuration is unsupported. The transport decision is not a replacement for administrator authentication or recovery authorization.
