...
When a manual video is started, the controller's local live-preview iframe is suspended to `about:blank`. Active receiver telemetry also keeps that preview suspended while video is playing. This prevents the controller browser from decoding a second copy of the MP4. The physical RoomGoblin receiver remains the playback authority.
