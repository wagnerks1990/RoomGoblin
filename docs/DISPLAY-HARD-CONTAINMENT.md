...
Renderer revision `dynamic-fit-20260922-11` makes containment and dynamic space use joint invariants.
...
Automatic fitting may grow short content only up to 10 percent above the configured size, still bounded by the absolute component safety cap, and must shrink dense content until every rendered glyph fits. Manual `autoFit:false` keeps the configured font size as a ceiling, but still never permits clipping.
...
The receiver HTML uses the same `dynamic-fit-20260922-11` cache key for the layout module and stylesheet. Every renderer behavior change must bump this key.
...
Expected revision: `dynamic-fit-20260922-11`.
