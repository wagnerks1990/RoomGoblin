...
- [AI CI Publication Context](ai/CI-PUBLICATION.md) — machine/assistant rules for exact-SHA validation, image publication, promotion, and production-branch advancement.
...
6. **Keep AI context current.** Architecture, installation-path, integration-health, scheduler, priority, managed-device capability, CI/publication, topology, or branding changes must be reflected in `AGENTS.md`/`AI-CONTEXT.md`/relevant `docs/ai/` context when they materially change how future work should be performed.
