...
test("GitHub main checks are bounded and select only trusted merged commits",()=>{
  const server=read("src/server.js");
  const check=server.match(/async function githubMainCheck\b[\s\S]*?\n\}/);
  assert.ok(check,"GitHub main checker was not found");
  const fetchHelper=server.match(/async function githubUpdateFetch\b[\s\S]*?\n\}/);
  assert.ok(fetchHelper,"bounded GitHub update fetch helper was not found");
  assert.match(fetchHelper[0],/(?:AbortSignal\.timeout|AbortController)/,"GitHub update requests must have a deadline");
...
test("System Updates GUI uses the native published-main transaction",()=>{
  ...
  assert.match(server,/action:"published",targetCommit:selected/,"Hub install route must hand the exact main commit to the native published action");
  assert.match(server,/confirm:"INSTALL_MAIN"/,"Hub install route must use explicit main-install confirmation");
  assert.match(server,/Selected main commit is no longer the current approved update/,"stale GUI selections must fail closed");
  assert.match(maintenance,/action:"published",targetCommit/,"maintenance bridge must preserve the published-main action");
  assert.match(host,/action not in \('update','published','revert'\)/,"Host Agent must explicitly allow published main updates");
  assert.match(host,/path=='\/app-updates\/source'/,"Host Agent must expose bounded source identity to the updater");
  assert.match(controller,/Checking trusted GitHub main for merged commits/);
  assert.match(controller,/body:JSON\.stringify\(\{commit\}\)/,"browser may submit only the previously resolved exact main commit");
  assert.match(html,/Main — merged commits/);
  assert.doesNotMatch(html,/option value="alpha">Alpha/,"semantic release channels must not remain selectable in the normal GUI");
});

test("installed update runners order after the canonical Host Agent service",()=>{
  const installer=read("install.sh"),runner=read("host-agent/app-update-runner.sh"),host=read("host-agent/server.py");
  const updateUnits=[...installer.matchAll(/Description=RoomGoblin (?:Native Host|Verified Application) Update Runner[\s\S]*?\nUNIT/g)].map(match=>match[0]);
  assert.equal(updateUnits.length,2,"both RoomGoblin update units must be generated by install.sh");
  for(const unit of updateUnits){
    assert.match(unit,/After=network-online\.target docker\.service classroom-hub-host-agent\.service/);
    assert.doesNotMatch(unit,/After=.*classroom-control-hub-host-agent\.service/);
  }
  assert.match(installer,/systemctl disable --now classroom-control-hub-host-agent\.service/,"installer must retire the stale old-name service");
  assert.match(runner,/systemctl disable --now classroom-control-hub-host-agent\.service/,"selective Host Agent refresh must retire the stale old-name service");
  assert.match(host,/"classroom-hub-host-agent\.service":\{"owner":"core"/,"Host Agent inventory must classify the canonical installed unit");
  assert.doesNotMatch(host,/"classroom-control-hub-host-agent\.service":\{"owner":"core"/,"stale service name must not remain the managed Host Agent identity");
});
