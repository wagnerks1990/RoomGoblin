...
test('native runner pulls/recreates only Hub and journals a verified rollback point',t=>{
 ...
 const status=JSON.parse(fs.readFileSync(f.state+'/app-update-status.json'));assert.equal(status.revertAvailable,true);assert.equal(status.previousCommit,f.base);assert.equal(status.phase,'completed');assert.equal(fs.existsSync(f.state+'/app-update-request.json'),false);assert.ok(r.events.includes("backups/retention"));assert.ok(r.events.includes("automaticKeep:3"));assert.ok(r.events.includes("preKeep:1"));assert.ok(r.events.includes("automatic:true"));
});
