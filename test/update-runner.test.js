...
test('native runner pulls/recreates only Hub and journals a verified rollback point',t=>{
 ...
 const status=JSON.parse(fs.readFileSync(f.state+'/app-update-status.json'));assert.equal(status.revertAvailable,true);assert.equal(status.previousCommit,f.base);assert.equal(status.phase,'completed');assert.equal(fs.existsSync(f.state+'/app-update-request.json'),false);assert.match(r.events,/backups\\/retention/);
});
