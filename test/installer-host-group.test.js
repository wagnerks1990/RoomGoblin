...
test("installer creates the Host Agent state root before restarting systemd service", () => {
  ...
  assert.match(installer, /CLOUDFLARED_UNIT=\/etc\/systemd\/system\/cloudflared-roomgoblin\.service/);
  assert.match(installer, /readlink "\$CLOUDFLARED_UNIT"\)" == \/dev\/null/);
  assert.match(installer, /\[\[ ! -e "\$CLOUDFLARED_UNIT" \]\]; then\n  install -m 0644 -o root -g root \/dev\/null "\$CLOUDFLARED_UNIT"/);
