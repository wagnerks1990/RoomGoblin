...
test('display renderer uses dynamic content-object layout', async () => {
  ...
  assert.equal(layout.LAYOUT_REVISION, 'dynamic-fit-20260921-3');
...
test('receiver cache key and build identity are release-stamped at image build', () => {
  ...
  assert.match(source, /layout\.mjs\?v=dynamic-fit-20260921-3/);
  assert.match(source, /layout\.css\?v=dynamic-fit-20260921-3/);
