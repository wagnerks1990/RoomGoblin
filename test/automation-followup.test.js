...
test("draft simulation is side-effect free",()=>{assert.match(server,/automations\/draft\/simulate/);const start=server.indexOf('automations/draft/simulate'),end=server.indexOf('app.get("/api/v1/displays/:id/media/status',start);const sim=server.slice(start,end);assert.doesNotMatch(sim,/runClassroomAutomation/);assert.doesNotMatch(sim,/executeCommand/);assert.match(controller,/simulateAutomationEditor/)});
