...
// The server checkpoints SQLite while draining. Never edit its database or
// reuse its listening port until its actual exit, even on a slow CI runner.
async function stop(child){
  if(!child||child.exitCode!==null||child.signalCode!==null)return;
  const exited=once(child,"exit");
  let forced=false;
  const deadline=setTimeout(()=>{forced=true;child.kill("SIGKILL")},12000);
  try{
    child.kill("SIGTERM");
    const [code,signal]=await exited;
    assert.equal(forced,false,"server must finish graceful shutdown before restart");
    assert.equal(signal,null,"server must exit normally");
    assert.equal(code,0,"server must close SQLite cleanly");
  }finally{clearTimeout(deadline)}
}

test("HTTP login preserves punctuation in passwords and startup repairs incomplete administrator capabilities",async(t)=>{
  ...
  t.after(async()=>{try{await stop(child)}finally{fs.rmSync(dir,{recursive:true,force:true})}});
  try{
  ...
  try{await wait(base,child)}finally{await stop(child)}
